...
#         self.vehicles_init = np.array(eval(rospy.get_param("/vehicle_init_posi", str([[0.0,0.0]] * self.vehicle_num)))).astype(np.float64)
...
#                 vehicle_posi_i = self.vehicles_posi[i].reshape([2,1])
...
M_pixel_to_carema = np.array([[1/fx, 0, -cx/fx], [0, 1/fy, -cy/fy], [0, 0, 1]])
Mrot  = np.array([[0, 0, 1], [-1, 0, 0], [0, -1, 0]]) 

pixel_coor = np.array([[u,v,1] for v in range(image_height) for u in range(image_width)]).T

points_pub = rospy.Publisher("/cloud_points", PointCloud2, queue_size=10)

map = np.zeros([1800, 1000, 70])
map_init = np.array([-40, -40, 0])
map_rslt = 0.1

def image_callback(image):
    global camera_rotm, camera_posi
    if camera_posi is not None and camera_rotm is not None:
        t0 = rospy.Time.now()
        bridge = CvBridge()
        depth_image = bridge.imgmsg_to_cv2(image, desired_encoding="passthrough").copy().flatten()
        points_camera = np.dot(M_pixel_to_carema, pixel_coor * depth_image[None, :])
        points_camera = points_camera[:, ~(np.isnan(depth_image) | (depth_image == 1))]
        Wxyz = np.dot(camera_rotm, np.dot(Mrot, points_camera)) + camera_posi.reshape([3,1])

        Oindex = np.floor((Wxyz - map_init.reshape([3,1]))/map_rslt).astype(int)
        cols = np.clip(Oindex[0,:], 0, 500-1)
        rows = np.clip(Oindex[1,:], 0, 500-1)
        tier = np.clip(Oindex[2,:], 0, 50 -1)
        map[rows, cols, tier] = 1

        clouds_index = np.where(map==1)
        clouds_x = clouds_index[1] * map_rslt + map_init[0]
        clouds_y = clouds_index[0] * map_rslt + map_init[1]
        clouds_z = clouds_index[2] * map_rslt + map_init[2]
        clouds_xyz = np.vstack([clouds_x, clouds_y, clouds_z]).T

        header = Header()
        header.stamp = rospy.Time.now()
        header.frame_id = "map"
        cloud = pc2.create_cloud_xyz32(header, clouds_xyz)
        points_pub.publish(cloud)
        print("publish")
        print("times: ", (rospy.Time.now()-t0).to_nsec())
        # cv2.imshow("Image window", (depth_image / np.max(depth_image) * 255).astype(np.uint8))
        # cv2.waitKey(1)  


def cpose_callback(camera_msg):
    global camera_rotm, camera_posi
    ...
    camera_posi = np.array([posi.x, posi.y, posi.z])



def MapBuild_callback(image_msg, camera_msg):
    orient = camera_msg.pose.orientation
    camera_rotm = tft.quaternion_matrix([orient.x, orient.y,orient.z,orient.w])[0:3,0:3]
    ...
    t0 = rospy.Time.now()
    bridge = CvBridge()
    depth_image = bridge.imgmsg_to_cv2(image_msg, desired_encoding="passthrough").copy().flatten()
    points_camera = np.dot(M_pixel_to_carema, pixel_coor * depth_image[None, :])
    points_camera = points_camera[:, ~(np.isnan(depth_image) | (depth_image == 1))]
    Wxyz = np.dot(camera_rotm, np.dot(Mrot, points_camera)) + camera_posi.reshape([3,1])
...
def image_subscriber():
    ...
    depth_image = Subscriber("/iris_0/realsense/depth_camera/depth/image_raw", Image)
    camera_pose = Subscriber("/iris_0/camera_pos", PoseStamped)

    ts = ApproximateTimeSynchronizer([depth_image, camera_pose], queue_size=10, slop=0.1, allow_headerless=True)
    ts.registerCallback(MapBuild_callback) 
